...
/*   Updated: 2020/05/24 20:33:05 by rtrant           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
									...
									size_t *string_size, char **number_to_print)
{
	...
	zeroes_count = directive->precision - ft_strlen(ft_itoa(ft_abs((long int)variable)));
	zeroes = zeroes_count >= 0 ? ft_calloc(zeroes_count + 1, sizeof(char)) :
	ft_strdup("");
	ft_memset(zeroes, '0', zeroes_count >= 0 ? zeroes_count : 0);
	*number_to_print = ft_strjoin(zeroes, ft_itoa(ft_abs(variable)));
